...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, **kwargs):
        ...
        self.max = nn.MaxPool3d(kernel_size=[2,2,2],stride=[2,2,2])
        self.conv1 = ConvLayer(in_channels, in_channels, kernel_size, 1, **kwargs)
        self.conv2 = ConvLayer(in_channels, out_channels, kernel_size, 1, **kwargs)
...
class UpsampleBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, **kwargs):
        ...
        self.transposed_conv=get_transp_conv(in_channels, in_channels, 2, 2, 3)
        #self.conv_block = ConvBlock(out_channels + in_channels, out_channels, kernel_size, 1, **kwargs)
        self.conv1 = ConvLayer(out_channels + in_channels, out_channels, kernel_size, 1, **kwargs)
...
class UNet3D(nn.Module):
    def __init__(
        ...
        self.norm = "batchnorm3d"
        self.filters = [64, 128, 256, 512][: len(strides)]
